...
var WalletCreateManager = function() {
    ...
    //生成地址 私钥 公钥 返回的结构为 { "address": address, "pubkey": pubkey, "privkey": privkey }
    this.address = function generateAddress(word, pass, coinType) {
        let obj = phraseChanged(word, pass, coinType);
        console.log("助记词" + word + " 密码" + pass + " 币种类型" + coinType);
        console.log("得到的结果" + JSON.stringify(obj));
        return obj;
    }

    //通过私钥得到地址,公钥 btc ltc 没问题
    this.addressFromWif = function generateAddressFromWif(Wif, coinType = "BTC - Bitcoin") {
        return getAddressAndPubkeyFromWif(Wif, coinType);
    }
     ...
     * @returns
     */
    function getAddressAndPubkeyFromWif(Wif, coinType) {

        let index = coinTypeEnum[coinType]
        ...
        var keyPair = bitcoinjs.bitcoin.ECPair.fromWIF(Wif, network);
        ...
        console.log("地址：" + address + " 公钥：" + pubkey);
    ...
    function displayBip32Info() {
        ...
        return row.generateAddressAndKey();
    }
    ...
    function getDerivationPath() {
        ...
        var coin = parseIntNoNaN(bip44coin, 0);
    ...
    function TableRow(index) {
        ...
        self.generateAddressAndKey = function calculateValues() {
